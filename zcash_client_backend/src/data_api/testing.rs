    ...
    <Cache::BlockSource as BlockSource>::Error: fmt::Debug,
{
    ...
    pub fn generate_block_at<Fvk: TestFvk>(
        ...
        if prior_cached_block.chain_state.block_height() == height - 1 {
            ...
            // Ironwood notes are not generated by callers that specify explicit tree sizes, so the
            // gap-filled prior block carries the prior Ironwood tree forward unchanged.
            #[cfg(feature = "orchard")]
            let ironwood_tree = prior_cached_block.chain_state.final_ironwood_tree().clone();
            let ironwood_end_size = prior_cached_block.ironwood_end_size();
