...
pub enum ProposalError {
    ...
    /// The turnstile only permits value to leave the pool: a step may not direct payments
    /// to the Orchard pool, and may return change to it only when strictly less value
    /// returns than the step's Orchard inputs remove.
    #[cfg(feature = "orchard")]
    OrchardPoolValueCreation {
        /// The total value of the Orchard notes spent by the step.
        input_total: Zatoshis,
        /// The total value of the Orchard-pool payment and change outputs created by the
        /// step.
        output_total: Zatoshis,
    },
}
...
impl<NoteRef> Step<NoteRef> {
    ...
    pub fn from_parts(
        ...
        if ironwood_active {
            ...
            let orchard_payment_total = payment_pools
                .iter()
                .filter(|(_, pool)| **pool == PoolType::ORCHARD)
                .map(|(idx, _)| {
                    transaction_request
                        .payments()
                        .get(idx)
            ...
            if orchard_payment_total.is_positive()
                || (orchard_change_total.is_positive()
                    && orchard_change_total >= orchard_input_total)
            {
                return Err(ProposalError::OrchardPoolValueCreation {
                    input_total: orchard_input_total,
                    output_total: (orchard_payment_total + orchard_change_total)
                        .ok_or(ProposalError::Overflow)?,
                });
            }
        }
...
mod tests {
    ...
    #[test]
    fn orchard_turnstile_rejects_orchard_pool_payments() {
        ...
        let step_for_pool = |pool: PoolType, ironwood_active| {
            Step::from_parts(
                &[],
                request.clone(),
        ...
        // Post-activation, a payment may not be directed to the Orchard pool even when the
        // step spends more Orchard value than the payment delivers.
        assert_matches!(
            step_for_pool(PoolType::ORCHARD, true),
            Err(ProposalError::OrchardPoolValueCreation {
                input_total,
                output_total,
            }) if input_total == Zatoshis::const_from_u64(10_000)
                && output_total == Zatoshis::const_from_u64(6_000)
        );

        // The same payment routed through the Ironwood bundle (the post-activation
        // representation of an Orchard-receiver payment) is valid.
        assert_matches!(step_for_pool(PoolType::IRONWOOD, true), Ok(_));

        // Before activation, an Orchard-pool payment is valid.
        assert_matches!(step_for_pool(PoolType::ORCHARD, false), Ok(_));
    }
